...
from .configuration_bitnet import BitnetAttentionConfig, BitnetFFNConfig, BitnetConfig
...
class BitnetMLP(nn.Module):
    def __init__(self, config):
        ...
        self.act_fn = ACT2FN[config.ffn_config.ffn_act_fn["name"]]
...
class BitnetAttention(nn.Module):
    ...
    def __init__(self, config: BitnetConfig, layer_idx: Optional[int] = None):
        ...
        self.attn_pdrop = config.attn_config.attn_pdrop
        self.hidden_size = config.d_model
        ...
        self.q_proj = BitLinear(
            self.hidden_size, self.num_heads * self.head_dim, bias=config.attn_config.bias
        )
        ...
        self.o_proj = BitLinear(
            self.hidden_size, self.hidden_size, bias=config.attn_config.bias
        )
        self._init_rope()

    def _init_rope(self):
        if self.config.attn_config.rope_scaling is None:
...
class BitnetSdpaAttention(BitnetAttention):
    """
...
class BitnetDecoderLayer(nn.Module):
    def __init__(self, config: BitnetConfig, layer_idx: int):
        ...
        self.self_attn = BITNET_ATTENTION_CLASSES[config._attn_implementation](
